    ...
    // Non-GPU particle field is handled in the in-scene objects section
        ...
        uint light_cluster_index = NUM_GRID_COMPONENTS * ((g.y * grid_size.x + g.x) * grid_size.z + g.z) + 1u;
            ...
            CALC_COLOR_MAP
                ...
                #if !defined(MISTY) && !defined(RAINY) && !defined(WET) && !defined(MISTY2) && !defined(WATER_HORIZON) && !defined(COASTAL) && !defined(SKY_ARK) && !defined(WATERFALL_ARENA)
            ...
            vec4 shadow_coords[4];
                ...
                return;
...

                    {
						...
						#if defined(COASTAL) && defined(TERRAIN)
                            if(world_vert.y < 16.9){
                                detail_fade = mix(detail_fade, 1.0, min(0.8, (16.9 - world_vert.y)*3.0));
                            }
                        #endif
                ...
                diffuse_color += ambient_color * GetAmbientContrib(shadow_tex.g) * ambient_mult * env_ambient_mult;
                    ...
                    #if defined(WATER_HORIZON) || defined(SNOW_EVERYWHERE) || defined(MORE_REFLECT) || defined(BEACH) || defined(COASTAL) || defined(SKY_ARK) || defined(WATERFALL_ARENA)
                    ...
                    /*
                        ...
                        spec_amount = spec_val;
                    */
                    #elif defined(METALNESS_PBR) || defined(ITEM) || defined(KEEP_SPEC) || defined(TERRAIN) || ((defined(RAINY) || defined(WET) || defined(SWAMP) || defined(BEACH) || defined(COASTAL) || defined(WATER_HORIZON) || defined(WATERFALL_ARENA) || defined(VOLCANO) || defined(SKY_ARK) || defined(SNOW_EVERYWHERE) || defined(SNOW_EVERYWHERE2) || defined(SNOW_EVERYWHERE3) || defined(DAMP_FOG) || defined(SWAMP2)) && !defined(WATER))
                        #if defined(RAINY) || defined(WET)
                            {
                            ...
                            const float kTintHeight = kBeachLevel+5;
                            ...
                            if(world_vert.y < kTintHeight ){
                                ...
                                colormap.xyz *= mix(vec3(1), vec3(92/255.0, 75/255.0, 20/255.0), opac*1);
                            } 
                    ...
                    #if defined(WATER_HORIZON) || defined(MORE_REFLECT) || defined(SNOW_EVERYWHERE) || defined(BEACH) || defined(COASTAL) || defined(SKY_ARK) || defined(WATERFALL_ARENA)
                        Caustics(kBeachLevel, ws_normal, diffuse_color);
                    #endif
                ...
                /*
                    ...
                    // out_color.xyz = spec_color.xyz;
                        ...
                        if(gl_FrontFacing){ // Only add foam and fog if water is viewed from outside of water
							...
							#if	defined(COASTAL) && !defined(SWAMP) && !defined(RAINY)
								float min_depth = -0.75;
                                float max_depth = 0.75;
                                float foam_detail = texture(tex0, (world_vert.xz + normalize(vec2(0.0, 1.0))*time*water_speed)*0.50).y +
                                texture(tex0, (world_vert.xz * 0.5 + normalize(vec2(1.0, 0.0))*time*water_speed)*6.0).y;
                                foam_detail *= 0.3;
                                foam_detail = min(1.0, foam_detail+0.4);

                                if(depth < max_depth && depth > min_depth && abs(old_depth - depth) < 1.50){ // Blend in foam
                                    if(depth > 0.0){
                                        under_water = mix(diffuse_color * 0.5, under_water, min(1, depth/max_depth + foam_detail));
                                    } else {
                                        under_water = mix(diffuse_color * 0.3, under_water, depth/-min_depth);
                                    }
                                }
                            #elif !defined(SWAMP) && !defined(RAINY)
                                float min_depth = -0.3;
                                float max_depth = 0.1;
                ...
                if(use_3d_tex){
                    ...
                    #if defined(MISTY) || defined(COASTAL) || defined(RAINY) || defined(WET) || defined(MISTY2) || (defined(WATER_HORIZON) && !defined(ALT2)) || defined(SKY_ARK) || defined(WATERFALL_ARENA)
                        val = 3.0;
                    #endif
