...
/*wtf*/ return ScaleNonUniform(bounding_box, obj.GetScale());
	
}
/* bool Fourtweny = false; */
void Update() {
    if(!co.controlled) {
    ...
    if(level.QueryIntFunction("int HasCameraControl()") == 1 || level.GetIntVar("display_text_id") == 696969){
        return;
    }else if(level.GetIntVar("display_text_id") >> 16 == 42069){
		int id = level.GetIntVar("display_text_id") & 65535;
		
		quaternion camrot = ReadObjectFromID(id).GetRotation();
		co.SetRotation(camrot);
		// Set camera euler angles from rotation matrix copypaste4lyf
        vec3 front = Mult(camrot, vec3(0,0,1));
        float yrt = atan2(front.x, front.z)*180.0f/MPI;
		float xrt = asin(front[1])*-180.0f/MPI;
        /* vec3 up = Mult(camrot, vec3(0,1,0));
		...
		rotation2 = target_rotation2 = floor(xrt*100.0f+0.5f)/100.0f;
		rotation = target_rotation = floor(yrt*100.0f+0.5f)/100.0f;

		position =	ReadObjectFromID(id).GetTranslation();
		Log(error,"42069 "+camera.GetPos().z);
		level.Execute("display_text_id = 0");
		return;
	}/* else if(Fourtweny){
		Fourtweny = false;
	} */
