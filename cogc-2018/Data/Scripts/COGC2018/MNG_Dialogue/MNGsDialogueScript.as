//copyright ©©©©© MNG 2018 Lmao Lugaru xDD™™™ is ™ so get of my property
#include "COGC2018/MNG_Dialogue/ANM-and-MORPH-Register.as" //For ANM paths in the load pose menu
...
enum ObjCommands {
    ...
    kSay,
    kFadeToBlack,
	cencordatcrapk, // okydoky gonna get dat meme in am tellin ya
	shotshotsshotshotsshotsheverybody, //as if
	setScamPos,
	exxxxxe,
	marker,
	jump,
...
};//why are there semicolons right here and at the end of the objcommands enumerator

class censoryword {
	//int displaychars
	string word;
	int dialoguetextindex;
	//string merlyn = "merlyn";
}

class MorphInfo : censoryword {//look at me, doin' the derivin magic (not bc it's particularly useful but bc I can)
	float weight;
	float weight_approach;
	bool Update_getdone(){
			if(!ObjectExists(dialoguetextindex)){
				return true;
			}
			ReadCharacterID(dialoguetextindex).rigged_object().SetMorphTargetWeight(word, weight_approach, 1.0f);
			weight_approach = mix(weight_approach,weight+0.05,0.09f);
			return weight_approach == weight;
	}
	MorphInfo(){}
	MorphInfo(string label, float _weight, int prtcpntno = dialogue.active_char, float _weight_weight=0f){
		word = label;
		dialoguetextindex = dialogue.GetDialogueCharID(prtcpntno);
		weight = _weight;
		weight_approach = _weight_weight;
	}
}
...
class CUTCN {
	...
	array<string> dialogue_name_fonts;
	array<string> dialogue_text_fonts;
    ...
    float cam_zoom;
	///////////////////////////
	quaternion gtfoffeulerrot;

	array<vec3> ScamPos;
	array<vec3> ScamRot;
	array<float> ScamZoom;

	int fromnode;
	int tonode;
	float interpltntime;
	float intrpltnstrtm;
	bool interpolaaaatioooooooonofpartyppl;
	bool memeskppable;
...
///////////////
	...
	array<censoryword> censrywords;
	int beebsndhndl;
	int stpbeepsndhndlindx = -1;

	bool No_Invisible_Strings= false;
	bool No_heal = false;
	array<MorphInfo> ToIntrpltMrphs;
    ...
    void UpdateRecordLocked(bool recording) {
        ...
        if(recording){
			...
			//i think i get it now. Those functions are hella weird lmaoooooo xfdddduAGPR2																				says the one with bool interpolaaaatioooooooonofpartyppl
			if(lstndapstrophepdate != -1){
                strings[lstndapstrophepdate].record_locked = true;
            }
    ...
    void ClearEditor() {
		...
		waiting_for_dialogue = false; //small change, big bug fix
		show_dialogue = false; //so the displayed stuff doesn't hang around after closing
		interpolaaaatioooooooonofpartyppl = false;
		ScamPos.resize(0);
		ScamRot.resize(0);
		ScamZoom.resize(0);

		censrywords.resize(0);
		memeskppable = true;

		waiting_for_choice = false;

		if(beebsndhndl != -1 ) {
			StopSound(beebsndhndl);
			beebsndhndl = -1;
		}
		stpbeepsndhndlindx = -1;
        ...
        if(preview) {
			...
			dialogue_name_fonts.resize(0);
			dialogue_text_fonts.resize(0);
			cstm_font_pth = font_path;
			cstm_nem_font_pth = name_font_path;
			No_Invisible_Strings = false;
			//sadly this would require custom character scripts, the recover happens when the char receives set dialogue control true msg
			No_heal = false;
        }
    }
    ...
    void Init() {
        ...
        //show_dialogue = false; am doing this in clear editor already.
        show_editor_info = true;
       // waiting_for_dialogue = false; am doing this in clear editor already.
    ...
    void PlayLineStartSound() {
        switch(GetActiveVoice()){
			...
			case 19: PlaySoundGroup("Data/Sounds/robloxjumprolletcc.xml", 0.3); break;
			case 20: PlaySoundGroup("Data/Sounds/horsded.xml", 0.3); break;
        }
    }
    ...
    void PlayLineContinueSound() {
        switch(GetActiveVoice()){
			...
			case 19: PlaySoundGroup("Data/Sounds/roblox.xml", 0.3); break;
			case 20: PlaySoundGroup("Data/Sounds/horse.xml", 0.3); break;
        }
    }
    ...
    string CreateStringFromParams(ObjCommands command, array<string> &in params){
		...
		case setScamPos:
			...
			break;
        case kCamera:          //no curly brackets? knew it.
            str = "set_cam";
            for(int i=0; i<num_params; ++i){
                str += " " + params[i];
            }
        ...
        case kCharacterDialogueControl:
            str = "send_character_message "+params[0]+" \"set_dialogue_control "+params[1]+(params.length()>2?(" "+params[2]):"")+"\"";
            break;
        case kCharacter:
    ...
    void AddInvisibleStrings() {
        ...
        str_params[1] = obj.GetTranslation().y+1;
		...
		str_params[0] = 1;
		//trnsltn
		str_params[1] = obj.GetTranslation().x;
        str_params[2] = obj.GetTranslation().y-1;
        str_params[3] = obj.GetTranslation().z;
		//rotatn
        str_params[4] = 0;
        str_params[5] = 0;
        str_params[6] = 0;
		//zoom
        str_params[7] = 90;
		AddLine(CreateStringFromParams(setScamPos, str_params), insert_index);
        strings[insert_index].visible = false;
		/////////////////////////////////////////

		//so you can always jump to the end without having to type markers
		str_params.resize(1);
        str_params[0] = "end_dialogue";
        int last_line = int(strings.size());
        AddLine(CreateStringFromParams(marker, str_params), last_line);
    ...
    bool SkipKeyDown() {
        if(!ready_to_skip || queue_skip || !HasCameraControl()){
            return false;
        }
    ...
    void Play(bool preventhardlockinloop = false) {
        bool stop = false;
    ...
    void RecordInput(const string &in new_string, int line, int last_wait) {
        if(new_string != strings[line].str){
            ...
            modified = true;
            int spwned_id = strings[line].spawned_id;
            if(strings[line].record_locked && last_wait > line){
                strings[line].spawned_id = -1;
                AddLine(new_string, selected_line);
                strings[selected_line-1].spawned_id = spwned_id;
                UpdateRecordLocked(IsRecording());
            } else if(line == selected_line || line == selected_line-1){
    ...
    int GetNextWait(int line) {
        ...
        }  is this bad copy paste? I don't get it.
		this returns the last wait before the end, not the next wait */
		for(int j=strings.size()-1; j>line; --j){
            if(strings[j].obj_command == kWaitForClick || strings[j].obj_command == kSay){
                last_wait = j;
            }
        }
	...
	///////////////maaan if this works you wouldn't even believe your eyes
	quaternion EulerDegToQuat(vec3 eul){
		return quaternion(vec4(0,1,0,eul.y*MPI/180)) * quaternion(vec4(1,0,0,eul.x*MPI/180)) * quaternion(vec4(0,0,1,eul.z*MPI/180));
	}
	//////////////
    void Update() {
		//Log(error,"cmctrl: "+(has_cam_control ? "true":"false")+" lol: "+fade_out_end);
        if(fade_out_end != -1.0){
            return;
        }
		...
		for(uint i = 0; i<ToIntrpltMrphs.length();++i){
			if(ToIntrpltMrphs[i].Update_getdone()){
				ToIntrpltMrphs.removeAt(i);
			}
		}
		//here comes the good stuff. not as good as multimemberment though haha good meme...																											no seriously when
        if(interpolaaaatioooooooonofpartyppl && fromnode >= 0 && tonode >= 0){

			gtfoffeulerrot = mix( EulerDegToQuat(ScamRot[fromnode]), EulerDegToQuat(ScamRot[tonode]), (the_time - intrpltnstrtm)/interpltntime);

            // Set camera euler angles from rotation matrix copypaste4lyf
            vec3 front = Mult(gtfoffeulerrot, vec3(0,0,1));
            float yrt = atan2(front.x, front.z)*180.0f/MPI;
			float xrt = asin(front[1])*-180.0f/MPI;
            vec3 up = Mult(gtfoffeulerrot, vec3(0,1,0));
            vec3 expected_right = normalize(cross(front, vec3(0,1,0)));
            vec3 expected_up = normalize(cross(expected_right, front));
            float zrt = atan2(dot(up,expected_right), dot(up, expected_up))*180.0f/MPI;

			cam_rot.x = floor(xrt*100.0f+0.5f)/100.0f;
			cam_rot.y = floor(yrt*100.0f+0.5f)/100.0f;
			cam_rot.z = floor(zrt*100.0f+0.5f)/100.0f;

			cam_pos = mix(ScamPos[fromnode], ScamPos[tonode], (the_time - intrpltnstrtm)/interpltntime);
			cam_zoom = mix(ScamZoom[fromnode], ScamZoom[tonode], (the_time - intrpltnstrtm)/interpltntime);
			// later, in the future when i'll finally get all this maff, i'll be sure to look back at this ;)

			if((the_time - intrpltnstrtm)/interpltntime > 1.0f){
				interpolaaaatioooooooonofpartyppl = false;
				memeskppable = true;
			}
		}
		...
		//I need 2 do dis since level.querryintfunction ignores custom level functions.
		//actually, gonna do this in the dnk_lvl.as script.
        if(has_cam_control){
			...
			//THIS 	somehow doesn't work, gonna take it to the levelscript
            /* if( waiting_for_choice || EditorModeActive()){
                SetGrabMouse(false);
            } */
        }
		...
		if(beebsndhndl != -1 && dialogue_text_disp_chars >= stpbeepsndhndlindx) {
			StopSound(beebsndhndl);
			beebsndhndl = -1;
			stpbeepsndhndlindx = -1;
		}
        ...
        if(SkipKeyDown() && dialogue_obj_id != -1){
            ...
            queue_skip = true;
            PlayLineStartSound();
        }																	  //need to prevent continuation sometime. I'm not using memeskppable bc i need waiting for choice in dnk_level.as
        if(GetInputPressed(controller_id, "attack") && start_time != the_time && memeskppable && !waiting_for_choice){
			//Log(error,"Click");
            if(index != 0){
            ...
            for(int i=0; i<int(strings.size()); ++i){
				...
				case setScamPos:
                    if(strings[i].spawned_id != -1){
                        ...
                        strings[i].params[4] = floor(x_rot*100.0f+0.5f)/100.0f;
                        strings[i].params[5] = floor(y_rot*100.0f+0.5f)/100.0f;
                        strings[i].params[6] = floor(z_rot*100.0f+0.5f)/100.0f;
                        strings[i].params[7] = zoom;

                        string new_string = CreateStringFromParams(strings[i].obj_command, strings[i].params);
                        RecordInput(new_string, i, last_wait);
                    } //ctr c ctr v is my friend. pls work first try.
                    break;
                case kHeadTarget:
                    if(strings[i].spawned_id != -1){
                        Object@ obj = ReadObjectFromID(strings[i].spawned_id);
    ...
    void ParseLine(ScriptElement &se){
        ...
        if(token == "set_cam"){
			...
			////////////////////////
		} else if(token == "send_HSP_msg"){
            se.obj_command = sendHSPmsg;

            token_iter.FindNextToken(msg);
            se.params.insertLast(token_iter.GetToken(msg));

			token_iter.FindNextToken(msg);
            se.params.insertLast(token_iter.GetToken(msg));
		} else if(token == "set_text_font"){
            se.obj_command = txtFont;
		...
		} else if(token == "set_name_font"){
            se.obj_command = nemFont;
		...
		} else if(token == "Intrpltn_Node"){
            se.obj_command = setScamPos;
		...
		} else if(token == "Char_exe"){
            se.obj_command = exxxxxe;
			...
			//////////////////
		} else if(token == "#No_Invisible_Strings"){
            No_Invisible_Strings = true;
		}else if(token == "#No_Heal"){
			//sadly this would require custom character scripts, the recover happens when the char receives set dialogue control true msg
			No_heal = true;
			////////////////////
			...
			se.obj_command = shotshotsshotshotsshotsheverybody;
			const int kNumParams = 4;
            se.params.resize(kNumParams);
            for(int i=0; i<kNumParams; ++i){
                token_iter.FindNextToken(msg);
			...
			se.obj_command = cencordatcrapk;
			se.params.resize(1);
			token_iter.FindNextToken(msg);
            se.params[0] = token_iter.GetToken(msg);
			//se.params[1] = dialogue_text; //for text metrics

			sub_strings.resize(sub_strings.size()+1);
			sub_strings[sub_strings.size()-1].str = "add_dialogue_text " + token_iter.GetToken(msg);
			ParseLine(sub_strings[sub_strings.size()-1]);
		} else if(token == "M") {
				se.obj_command = morph;
				token_iter.FindNextToken(msg);
				se.params.insertLast(token_iter.GetToken(msg));
				token_iter.FindNextToken(msg);
				se.params.insertLast(token_iter.GetToken(msg));
    ...
    void CreateEditorObj(ScriptElement@ se){
		...
		case setScamPos:{
            ...
            vec3 rot(atof(se.params[4]), atof(se.params[5]), atof(se.params[6]));
            float zoom = atof(se.params[7]);
			zoom = zoom <= 0.0f ? 90.0f : zoom;
            ...
            quaternion rot_z(vec4(0,0,1,rot.z*deg2rad));//hmmm why did I not notice this when I needed Eulerang to quat conversion
			...
			//we need some distinguishableness (how2inglish)
			//obj.SetTint(vec3(0.5,0.9,0.3)); fuck dat, doesn't work. Gonna have to live with the tag fr nw
			...
			obj.SetEditorLabel("IpltnNd "+atoi(se.params[0]));
            obj.SetEditorLabelScale(6);
            obj.SetEditorLabelOffset(vec3(0, 0.2f, 0.0f));
			break;}
        case kCamera: {
            if(se.spawned_id == -1){
                se.spawned_id = CreateObject("Data/Objects/placeholder/camera_placeholder.xml", true);
    ...
    void AnalyzeForLineBreaks(string &inout str){
        TextMetrics metrics = GetTextAtlasMetrics(cstm_font_pth, GetFontSize(), 0, dialogue_text);
        ...
        while(first_line.length() > 0){
            while(metrics.bounds_x > threshold){
                ...
                metrics = GetTextAtlasMetrics(cstm_font_pth, font_size, 0, first_line);
            }
            final += first_line + "\n";
    ...
    bool ExecuteScriptElement(const ScriptElement &in script_element, ProcessStringType type) {
		...
		case morph:{
				if(script_element.params.length() > 2){
					ToIntrpltMrphs.insertLast(MorphInfo(script_element.params[0],atof(script_element.params[2]),atoi(script_element.params[1])));
				}else{
					ToIntrpltMrphs.insertLast(MorphInfo(script_element.params[0],atof(script_element.params[1])));
				}/*
				if(GetDialogueCharID(prtcpnt_no) != -1){
					Log(error,"setting morph "+script_element.params[0]+" ("+weight+") for prtcpnt number "+prtcpnt_no+" with ID "+GetDialogueCharID(prtcpnt_no));
				} */
			}
			break;
		case sendHSPmsg:
			ReadObjectFromID(atoi(script_element.params[0])).ReceiveScriptMessage(script_element.params[1]);
			break;
		case choic:
			if(type != kInGame){
				break;
			}
			addChoiceButtons(script_element.params);
			waiting_for_choice  = true;
			return true;
		case jump:
			if(type == kInGame){
				int numprms = script_element.params.length();

				int markerIndx = 0;
				while(markerIndx != -1 && (strings[markerIndx].obj_command != marker || strings[markerIndx].params[0] != script_element.params[0])){
					markerIndx = (markerIndx >= int(strings.size()-1)) ? -1 : ++markerIndx; //butiful
				}

				bool condsTru = false;
				ScriptParams@ lvlscrptprmrs = level.GetScriptParams();

				if(numprms > 1){
					if(lvlscrptprmrs.HasParam(script_element.params[1])){
						if(numprms > 2){
							condsTru = lvlscrptprmrs.GetString(script_element.params[1]) == script_element.params[2];
						}else{
							condsTru = true;
						}
					}
				...
				if(markerIndx != -1 && condsTru && !(queue_skip && markerIndx <= index)){//anti loop hard lock
					index = markerIndx;
					Log(error, "jumping to: "+script_element.params[0]+", index: "+index);
				}else{
					Log(error, "No jump to "+script_element.params[0]+", conditions: "+(condsTru ? "true":"false")+", markerindex: "+markerIndx+", proceeding with script.");
				}
			}
		break;
		case exxxxxe:
            if(type == kInGame){
				int id = GetDialogueCharID(atoi(script_element.params[0]));
                if(id != -1){
                    MovementObject@ mmo = ReadCharacterID(id);
					mmo.Execute(script_element.params[1]);
                }
            }
            break;
        case kSetCamControl:
            ...
            break;
		case cencordatcrapk: ///allllllrightyright
			if(type != kInGame){
				break;
			}
			censrywords.resize(censrywords.size()+1);

			censrywords[censrywords.size()-1].dialoguetextindex = dialogue_text.length();//problem changed, solution  changed :C dialogue_text.substr(max(dialogue_text.findLast("\n"),0)); //I feel like a fucking genius. This is easily my favourite line!
			censrywords[censrywords.size()-1].word = script_element.params[0];
			/* while(censrywords[censrywords.size()-1].merlyn.length()<censrywords[censrywords.size()-1].word.length()){
				censrywords[censrywords.size()-1].merlyn.insert(5,"y");
			} */
			if(beebsndhndl==-1 && !queue_skip && !skip_dialogue){
				beebsndhndl = PlaySoundLoop("Data/Sounds/cnsrbp.wav", 40.0f);
				stpbeepsndhndlindx = dialogue_text.length() + script_element.params[0].length();
			}else{
				StopSound(beebsndhndl);
				beebsndhndl = -1;
			}
			/* for(uint i =0; i< censrywords.size();++i){
				Log(error, censrywords[i].word+" "+censrywords[i].dialoguetext);
			} */
            break;
		case shotshotsshotshotsshotsheverybody:
			if(type != kInGame){
				break;
			}
			interpolaaaatioooooooonofpartyppl = true;
			fromnode = atoi(script_element.params[0]);
			tonode = atoi(script_element.params[1]);

			interpltntime = max(atof(script_element.params[2]),0.00001f);
			intrpltnstrtm = the_time;

            memeskppable = (script_element.params[3] != "false");

			break;
		case setScamPos:
			if(atoi(script_element.params[0]) > 9 || atoi(script_element.params[0]) < 0){
				Log(error, "Index for Intrpltn Node to high or too low my dear. (0-10)");
				break;
			}
			if (int(ScamPos.length()) < atoi(script_element.params[0])+1){
				ScamPos.resize(atoi(script_element.params[0])+1);
				ScamRot.resize(atoi(script_element.params[0])+1);
				ScamZoom.resize(atoi(script_element.params[0])+1);
			}

			ScamPos[atoi(script_element.params[0])].x = atof(script_element.params[1]);
			ScamPos[atoi(script_element.params[0])].y = atof(script_element.params[2]);
			ScamPos[atoi(script_element.params[0])].z = atof(script_element.params[3]);

			ScamRot[atoi(script_element.params[0])].x = atof(script_element.params[4]);
			ScamRot[atoi(script_element.params[0])].y = atof(script_element.params[5]);
			ScamRot[atoi(script_element.params[0])].z = atof(script_element.params[6]);

			ScamZoom[atoi(script_element.params[0])] = atof(script_element.params[7]);
			//don't really like the way I handeled this. an array of a class with 2 vec3s and one float would've been more elegant than three arrays I guess
			/* for(uint i=0; i< ScamPos.size(); ++i ){
				Log(error, "ScamRot_x nr "+i+": "+ScamPos[i].x);
				Log(error, "ScamRot_y nr "+i+": "+ScamPos[i].y);
				Log(error, "ScamRot_z nr "+i+": "+ScamPos[i].z);

				Log(error, "ScamPos_x nr "+i+": "+ScamPos[i].x);
				Log(error, "ScamPos_y nr "+i+": "+ScamPos[i].y);
				Log(error, "ScamPos_z nr "+i+": "+ScamPos[i].z);

				Log(error, "ScamZoom nr "+i+": "+ScamZoom[i]);
			} */
			/* Log(error, "ScamPos_size: "+ScamPos.size());
			Log(error, "ScamRot_size: "+ScamRot.size());
			Log(error, "ScamZoom_size: "+ScamZoom.size()); */
			break;
        case kCamera:
            if(type == kInGame){
                // Set camera control
        ...
        case kDialogueVoice: {
            ...
            break;
			//////////////////font customization, why not
		case nemFont:{
            int id = atoi(script_element.params[0])-1;
            if(id >= 0){
                if(id >= int(dialogue_name_fonts.size())){
                    dialogue_name_fonts.resize(id+1);
                }
                dialogue_name_fonts[id] = script_element.params[1];
            } }
			break;
		case txtFont:{
            int id = atoi(script_element.params[0])-1;
            if(id >= 0){
                if(id >= int(dialogue_text_fonts.size())){
                    dialogue_text_fonts.resize(id+1);
                }
                dialogue_text_fonts[id] = script_element.params[1];
            } }
			break;
			////////////////////
        case kSay:
    ...
    void ReceiveMessage(const string &in msg) {
		...
		////////////
		if(token == "dlg_choice"){
			if(!waiting_for_choice || !token_iter.FindNextToken(msg)){
				return;
			}

			int markerIndx = 0;
			while(markerIndx != -1 && (strings[markerIndx].obj_command != marker || strings[markerIndx].params[0] != token_iter.GetToken(msg))){
				markerIndx = (markerIndx >= int(strings.size()-1)) ? -1 : ++markerIndx; //butiful
			}
			if(markerIndx != -1){
				index = markerIndx;
			}else{
				Log(error, "No marker with name '"+token_iter.GetToken(msg)+"' found, proceeding script w/out jump");
			}
			waiting_for_choice = false;
			Play();

        }else if(token == "notify_deleted"){
		    token_iter.FindNextToken(msg);
        ...
        } else if(token == "stop_editing_ctc") {
            DeactivateKeyboardEvents();
            ClearEditor();
        } else if(token == "edit_ctc"){
		...
		} else if(token.findFirst("edit_ctc_id") != -1) {
            Log(info, "Got edit_ctc_id");
            ...
            if(id != -1){
                ...
                if(last_set_animation == -1 || last_set_animation < last_wait){
                    AddLine(CreateStringFromParams(kSetAnimation, str_params), selected_line);
                } else if(selected_line == last_set_animation){
                    strings[last_set_animation].str = CreateStringFromParams(kSetAnimation, str_params);
                    ParseLine(strings[last_set_animation]);
                } else{
					AddLine(CreateStringFromParams(kSetAnimation, str_params), selected_line);
					strings.removeAt(last_set_animation);
					selected_line--;
				}
                UpdateScriptFromStrings();
	...
	/////////////
	/* void InlinePrevious(int curserpos){


	} */

	string cstm_font_pth; //= font_path;
	string cstm_nem_font_pth; //= name_font_path;
    ...
    void Display() {
        ...
        if(show_dialogue && (/* camera.GetFlags() == kPreviewCamera */ preview || !EditorModeActive())){ // this is needed so no invstrings wdialogue is displayed as well
            int font_size = GetFontSize();
			...
			if(active_char >= 0 && int(dialogue_name_fonts.size()) > active_char && FileExists(dialogue_name_fonts[active_char])){
                cstm_nem_font_pth = dialogue_name_fonts[active_char];
            } else {
                cstm_nem_font_pth = font_path;
            }
            ...
            if(fade_end != -1.0){
                ...
                if(dialogue_name != ""){//don't wanna see the brush stroke if there's no text. useful for monologue in dating sim. (jeesus christ)
                    HUDImage @blackout_image = hud.AddImage();
                    TextMetrics metrics = GetTextAtlasMetrics(cstm_nem_font_pth, int(GetFontSize()*1.8), kSmallLowercase, dialogue_name);
        ...
        if(dialogue_obj_id != -1 && show_editor_info && !has_cam_control && EditorModeActive()){
            ...
            ImGui_Begin("MNG's Dialogue Editor - " + params.GetString("DisplayName") + (modified ? "*" : "") + "###Dialogue Editor", show_editor_info, ImGuiWindowFlags_MenuBar);
            vec2 previewindowpos = ImGui_GetWindowPos()-vec2(275,0);
			if(ImGui_BeginMenuBar()){
                if(ImGui_BeginMenu("File")){
					if(ImGui_MenuItem("Load Script")){
                        modified = LoadScriptFile(GetUserPickedReadPath("txt", "Data/Dialogues"));
						UpdateScriptFromStrings();
                    }
                ...
                if(ImGui_BeginMenu("Edit")){
                    ...
                    if(ImGui_BeginMenu("Poses")){
						...
						ImGui_Separator();
						for(uint i=0;i<Anms.size();i++){
							if(ImGui_MenuItem(Anms[i].Name)){
								ReceiveMessage("load_dialogue_pose \""+Anms[i].Path+"\"");
								modified = true;
							}
							if(ImGui_IsItemHovered()) {
								ImGui_SetNextWindowPos(previewindowpos);
								ImGui_Begin("",ImGuiWindowFlags_NoResize + ImGuiWindowFlags_NoTitleBar + ImGuiWindowFlags_NoFocusOnAppearing);
								ImGui_Image(LoadTexture(Anms[i].Previewimagepath), vec2(256,256));
								ImGui_End();
							}
						}
						ImGui_EndMenu();
                    }
					if(ImGui_MenuItem("Inline previous")){
                        /* InlinePrevious(imgui_text_input_CursorPos); */
						if(selected_line > 0 && strings[selected_line].obj_command == kSay){
							/*i'm gonna make a comment about this, it's too long to not do it
							first, it takes the string of the previous command (scriptelement.str) and escapes it
							then it inserts the escaped string into the string of the say command
								position is important. the position is retrieved by subtracting the start char in the
								full script (scriptelement.line_start_char) from the cursor position in the full script*/
							strings[selected_line].str.insert(imgui_text_input_CursorPos-strings[selected_line].line_start_char, '['+EscapeString(strings[selected_line-1].str)+']');//holy mother of god
							//the .str is changed now, but the params aren't, we need to parse this element again
							ParseLine(strings[selected_line]);
							strings.removeAt(selected_line-1);
							selected_line--;
							UpdateScriptFromStrings();
							modified = true;
						}
				...
				if(selected_line > -1 && selected_line < int(strings.length()) && (strings[selected_line].obj_command == kCamera || strings[selected_line].obj_command == setScamPos)){
					if(ImGui_Button('Cam to View')){
						ReadObjectFromID(strings[selected_line].spawned_id).SetTranslationRotationFast(camera.GetPos(), EulerDegToQuat(vec3(camera.GetXRotation(),camera.GetYRotation(),camera.GetZRotation())));
						UpdateScriptFromStrings();
					}
					if(ImGui_Button('View to Cam')){
						int numnid = 42069 << 16;
						numnid |= strings[selected_line].spawned_id;

						level.Execute("display_text_id = "+numnid);
					}/* else if(level.GetIntVar("display_text_id") == 420420){
						//this is """"pretty""" elegant (it's not actually, just relative to what I'm gonna do now), but not reliable at all.
						//will transfer this logic to cam.as and pass the id via display_text_id, somehow (or maybe use another unused variaable in level.as?)
						int lth = strings[selected_line].params.length();
						camera.SetPos(vec3(atof(strings[selected_line].params[lth-7]),atof(strings[selected_line].params[lth-6]),atof(strings[selected_line].params[lth-5])));
						camera.SetXRotation(atof(strings[selected_line].params[lth-4]));
						camera.SetYRotation(atof(strings[selected_line].params[lth-3]));
						camera.SetZRotation(atof(strings[selected_line].params[lth-2]));
						level.Execute("display_text_id = -1");
					} */
				}
                ImGui_EndMenuBar();
            }
            ...
            if(!show_editor_info){ // User closed dialogue editor window
                ...
                if(ImGui_InputTextMultiline("##TEST", vec2(-1.0, -1.0))){
                    ...
                    AddInvisibleStrings();
                    modified = true;
					//make doublecklick to select one word work
					/* if(ImGui_IsItemHovered() && ImGui_IsMouseDoubleClicked(0)){
						string txt = ImGui_GetTextBuf();
						this doesn't work. sadface
						imgui_text_input_SelectionStart =  1 + txt.findLast (" ", imgui_text_input_CursorPos);
						imgui_text_input_SelectionEnd	= -1 + txt.findFirst(" ", imgui_text_input_CursorPos);
					} */
                }
	...
	///////////////oof IMGUI
	void addChoiceButtons(array<string> chcs){

			imGUI.clear();
			imGUI.setup();

			int numchs = chcs.length();

			IMContainer bttncntnr(GetScreenWidth()*0.95f,GetScreenHeight()*0.15f);
			bttncntnr.setAlignment(CACenter, CABottom);

			IMDivider buttons_holder(DOHorizontal);
			buttons_holder.setSizeX(GetScreenWidth()*0.95f);
			buttons_holder.setSizeY(GetScreenHeight()*0.15f);
			buttons_holder.setBorderColor(vec4(1,0,0,1));

			buttons_holder.append(IMSpacer(DOHorizontal, 25.0f));

			buttons_holder.setAlignment(CACenter,CABottom);


			for(int i=0;i<numchs;++i){
				IMMessage@ bttnmsg = IMMessage("dlg_choice",'"'+chcs[i]+'"');
				AddButton(	chcs[i],
									buttons_holder,"null",button_background_diamond,true,default_button_width,text_trailing_space,move_button_background,//filler stuff
							bttnmsg);
			}

			bttncntnr.setElement(buttons_holder);
			imGUI.getMain().setElement(@bttncntnr);
    ...
    void Display2() {
        ...
        if(show_dialogue && (/* camera.GetFlags() == kPreviewCamera */ preview || !EditorModeActive())){ // this is needed so no invstrings wdialogue is displayed as well
			...
			/* if(NameInfoExists(dialogue_name)){
                color = GetNameInfo(dialogue_name).color;
            } else */ if(active_char >= 0 && int(dialogue_text_fonts.size()) > active_char && FileExists(dialogue_text_fonts[active_char])){
                cstm_font_pth = dialogue_text_fonts[active_char];
            } else {
                cstm_font_pth = font_path;
            }
            ...
            DrawTextAtlas(cstm_nem_font_pth, int(font_size*1.8), kSmallLowercase, dialogue_name,
            ...
            DrawTextAtlas2(cstm_font_pth, font_size, 0, dialogue_text,
                          int(pos.x)+font_size, int(pos.y)+font_size, vec4(vec3(1.0f), 1.0f), int(dialogue_text_disp_chars));
            TextMetrics test_metrics = GetTextAtlasMetrics2(cstm_font_pth, GetFontSize(), 0, dialogue_text, int(dialogue_text_disp_chars));
            if(!waiting_for_dialogue && !is_waiting_time && test_metrics.bounds_y < GetFontSize() * 3){
                TextMetrics metrics = GetTextAtlasMetrics(cstm_font_pth, GetFontSize(), 0, continue_string);
                DrawTextAtlas(cstm_font_pth, font_size, 0, continue_string,
                               GetScreenWidth() - int(kTextRightMargin) - metrics.bounds_x, int(pos.y)+font_size*4, vec4(vec3(1.0f), 0.5f));
            }
			////cooomin in i have no idea how text metrics works what is advance.x i wanna know kool kool edit: I actually know now, it's really useful xd
				for(uint i = 0;i<censrywords.size();++i){
                    HUDImage @censordiswereaeforeveryonegemhere = hud.AddImage();
                    TextMetrics dialtxtmetrics = GetTextAtlasMetrics(cstm_font_pth, GetFontSize(), 0, dialogue_text.substr(0,censrywords[i].dialoguetextindex));
					TextMetrics wrdtcnsrtxtmetrics = GetTextAtlasMetrics(cstm_font_pth, GetFontSize(), 0, censrywords[i].word);

                    censordiswereaeforeveryonegemhere.SetImageFromPath("Data/Textures/censordtk.png");
                    censordiswereaeforeveryonegemhere.position.y = GetScreenHeight()-int(pos.y)-(font_size/4)-dialtxtmetrics.bounds_y;
                    censordiswereaeforeveryonegemhere.position.x = int(pos.x)+font_size+dialtxtmetrics.advance_x;
                    censordiswereaeforeveryonegemhere.position.z = -1.0f;
                    censordiswereaeforeveryonegemhere.scale = vec3((wrdtcnsrtxtmetrics.bounds_x)/160.0, font_size/80.0, 1.0f);
                    censordiswereaeforeveryonegemhere.color = vec4(vec3(1.0f),min(dialogue_text_disp_chars - censrywords[i].dialoguetextindex - 0.5*censrywords[i].word.length(),1.0f));

					/* DrawTextAtlas2(font_path,
									font_size,
									0,
									censrywords[i].merlyn,
									int(pos.x)+font_size+dialtxtmetrics.bounds_x,
									int(pos.y)+dialtxtmetrics.bounds_y, vec4(vec3(0.0f), 1.0f),
									max(dialogue_text_disp_chars-censrywords[i].dialoguetext.length()- 0.9f*censrywords[i].word.length(),
										censrywords[i].merlyn.length())); */
                }
            //it fu*merliiin*ing worked?? never would i've thought that this shit 'd work that fukkin easily ma dear
            LeaveTelemetryZone();
        }
    }
	...
	/*  void Display3() {
		...
		vec2 pos(kTextLeftMargin, GetScreenHeight() *0.75 + GetFontSize() * 1.2);
		for(uint i = 0;i<censrywords.size();++i){
                TextMetrics dialtxtmetrics = GetTextAtlasMetrics(font_path, GetFontSize(), 0, censrywords[i].dialoguetext);
				TextMetrics wrdtcnsrtxtmetrics = GetTextAtlasMetrics(font_path, GetFontSize(), 0, censrywords[i].word);

				DrawTextAtlas2(font_path,
								GetFontSize(),
								0,
								censrywords[i].merlyn,
								int(pos.x)+GetFontSize()+dialtxtmetrics.bounds_x,
								int(pos.y)+dialtxtmetrics.bounds_y, vec4(vec3(0.0f), 1.0f),
								max(dialogue_text_disp_chars-censrywords[i].dialoguetext.length()- 0.9f*censrywords[i].word.length(),
									censrywords[i].merlyn.length()));
               }
