...
bool on__achievment = false;
string achievement;
int achievrepeats;
int strikerepeats;
int nailedittimes = 0;
int spaduzzledittimes = 0;

int ingame_text_id = -1;
float rotation;

string default_death_message = " Press $attack$ to try again";
...
void SetStingIfNotSet(bool norm_mode, string succpath = "nukk", string dthpth = "nukk"){
	if(norm_mode){
		dthpth = "Data/Music/"+succpath+"_loop/"+succpath+"_defeat.wav";//in this case, order matters. yes. not elegant, but efficient
		succpath = "Data/Music/"+succpath+"_loop/"+succpath+"_success.wav";
	}
	if(params.GetString("Dth_Stng") == "nukk") {
			params.SetString("Dth_Stng", dthpth);
	}
	if(params.GetString("Succ_Stng") == "nukk") {
		params.SetString("Succ_Stng", succpath);
	}
}
...
void SetParameters() {
	...
	params.AddString("Succ_Stng","nukk");
	params.AddString("Dth_Stng","nukk");
    params.AddString("death_message", default_death_message);
    params.AddIntCheckbox("press_attack_to_restart", true);
	
	TokenIterator mscprm_iter;
	mscprm_iter.Init();
	mscprm_iter.FindNextToken(params.GetString("music"));
	string sngnmorpth = mscprm_iter.GetToken(params.GetString("music"));
	if(FileExists(sngnmorpth) && mscprm_iter.FindNextToken(params.GetString("music"))){
		AddMusic(sngnmorpth);
		PlaySong(mscprm_iter.GetToken(params.GetString("music")));
		music_prefix = mscprm_iter.GetToken(params.GetString("music"))+"_";
		
	} else if(sngnmorpth == "slaver"){
        AddMusic("Data/Music/slaver_loop/layers.xml");
        PlaySong("slavers1");
        music_prefix = "slavers_";
		//need this because slaver files aren't named according to norm  
		SetStingIfNotSet(false,"Data/Music/slaver_loop/the_slavers_success.wav","Data/Music/slaver_loop/the_slavers_defeat.wav");
		
    } else if(sngnmorpth == "swamp"){
        AddMusic("Data/Music/swamp_loop/swamp_layer.xml");
        PlaySong("swamp1");
        music_prefix = "swamp_";
		SetStingIfNotSet(true,sngnmorpth);
		
    } else if(sngnmorpth == "cats"){
        AddMusic("Data/Music/cats_loop/layers.xml");
        PlaySong("cats1");
        music_prefix = "cats_";
		SetStingIfNotSet(true,sngnmorpth);
		
    } else if(sngnmorpth == "crete"){
        AddMusic("Data/Music/crete_loop/layers.xml");
        PlaySong("crete1");
        music_prefix = "crete_";
		SetStingIfNotSet(true,sngnmorpth);
		
    } else if(sngnmorpth == "rescue"){
        AddMusic("Data/Music/rescue_loop/layers.xml");
        PlaySong("rescue1");
        music_prefix = "rescue_";
		SetStingIfNotSet(true,sngnmorpth);
		
    } else if(sngnmorpth == "arena"){
        AddMusic("Data/Music/SubArena/layers.xml");
        PlaySong("sub_arena");
        music_prefix = "arena_";
		///////////////////////////////////////////////
	}else if (sngnmorpth == "lmao"){
		AddMusic("Data/XMLStuff/msc/lmaomsc.xml");
        PlaySong("lmaao");
        music_prefix = "lmao_";
...
void handleUpdateVarNeedingGoals(int goalnr){
	TokenIterator token_iter;
	if(params.HasParam("goal_"+goalnr)){
        string goal_str = params.GetString("goal_"+goalnr);
        token_iter.Init();
		if(token_iter.FindNextToken(goal_str)){
			if(token_iter.GetToken(goal_str) == "reach_timed"){
				token_iter.FindNextToken(goal_str);
				token_iter.FindNextToken(goal_str);
				ttimermng = atof(token_iter.GetToken(goal_str));
				if(token_iter.FindNextToken(goal_str)){
					on__time = (token_iter.GetToken(goal_str)=="strtdrctly");
					if(on__time){ttimermng = the_time + ttimermng;}
				} else {on__time = false;}
				
				on__achievment= false;
				nailedittimes = 0;
				spaduzzledittimes = 0;
				
			} else if(token_iter.GetToken(goal_str) == "achievement") {
				on__achievment = token_iter.FindNextToken(goal_str);
				achievement = token_iter.GetToken(goal_str);
				token_iter.FindNextToken(goal_str);
				achievrepeats = atoi(token_iter.GetToken(goal_str));
				token_iter.FindNextToken(goal_str);
				strikerepeats = atoi(token_iter.GetToken(goal_str));
...
void omgachhivgoalsarkinnacmplctdt(string tkn, int charid_a, int charid_b){ //this will require some rework to work how i'd like it to

	//if(charid_a == -1){return;}
	/* Log(info,	"achiv "+achievement+"\n"
				"repeats "+achievrepeats+"\n"
				"strikerepeats "+strikerepeats+"\n"
				"nailed "+nailedittimes+"\n"
				"spdizzled"+spaduzzledittimes); */
	if(tkn == achievement){
		++nailedittimes;
	} else{
		++spaduzzledittimes;
	}
	if(spaduzzledittimes > strikerepeats){
		if(params.HasParam("goal_"+progress+"_lost")){
			TriggerGoalString(params.GetString("goal_"+progress+"_lost"),_all_but_sting);
		} else {
			level.SendMessage("reset"); 
		}
	}
	if(nailedittimes >= achievrepeats){
...
void TriggerGoalString(const string &in goal_str, GoalTriggerType type){
    ...
    while(token_iter.FindNextToken(goal_str)){
		 if(token_iter.GetToken(goal_str) == "plysng" && !EditorModeActive()){
            if(token_iter.FindNextToken(goal_str) && type == _all_but_sting){
                PlaySong(token_iter.GetToken(goal_str));
            }
        }
        if(token_iter.GetToken(goal_str) == "ctc" && !EditorModeActive()){
            if(token_iter.FindNextToken(goal_str) && type == _all_but_sting){
                level.SendMessage("cutcn \""+token_iter.GetToken(goal_str)+"\"");
            }
        }
        if(token_iter.GetToken(goal_str) == "ctc_fade" && !EditorModeActive()){
            if(token_iter.FindNextToken(goal_str) && type == _all_but_sting){
                level.SendMessage("cutcn_fade \""+token_iter.GetToken(goal_str)+"\"");
            }
        }
        if(token_iter.GetToken(goal_str) == "ctc_fade_no_aggr" && !EditorModeActive()){
            if(token_iter.FindNextToken(goal_str) && type == _all_but_sting){
		...
		if(token_iter.GetToken(goal_str) == "play_sting" && !EditorModeActive()){
            if(type ==_all_but_sting && token_iter.FindNextToken(goal_str)){
               PlayRandSting(atoi(token_iter.GetToken(goal_str)));
            }
        }
		if(token_iter.GetToken(goal_str) == "playsong" && !EditorModeActive()){
            if(type == _all_but_sting && token_iter.FindNextToken(goal_str)){
                PlaySong(token_iter.GetToken(goal_str));
            }
        }
...
void PlaySuccessSting() {
	...
	if(FileExists(params.GetString("Succ_Stng"))){
		sting_handle = PlaySound(params.GetString("Succ_Stng"));
...
void PlayDeathSting() {
	...
	/* array<string> @fuu = GetLayerNames();
	for(uint i = 0; i<fuu.size(); i++) {
		Log(error, fuu[i]);
	} */
	
	if(FileExists(params.GetString("Dth_Stng"))){
		sting_handle = PlaySound(params.GetString("Dth_Stng"));
		SetSoundGain(sting_handle, GetConfigValueFloat("music_volume"));
		music_sting_end = the_time + 5.0;
		SetLayerGain(music_prefix+"layer_"+curr_music_layer, 0.0);
	}	
...
void PlayRandSting(int nrt){
	if(params.HasParam("Sting_"+nrt)){
		if(sting_handle != -1){
			StopSound(sting_handle);
			sting_handle = -1;
		}
		if(FileExists(params.GetString("Sting_"+nrt))){
			sting_handle = PlaySound(params.GetString("Sting_"+nrt));
			SetSoundGain(sting_handle, GetConfigValueFloat("music_volume"));
			music_sting_end = the_time + 15.0;
			SetLayerGain(music_prefix+"layer_"+curr_music_layer, 0.0);
		}
	}
...
void IncrementProgress() {
	...
	handleUpdateVarNeedingGoals(progress);
    ...
    if(event == "checkpoint"){
        ...
        if(params.HasParam("goal_"+goal_check)){
            ...
            if(token_iter.FindNextToken(goal_str)){
                ...
                if(goal_type == "reach" || goal_type == "reach_skippable" || goal_type == "reach_timed"){
                    if(token_iter.FindNextToken(goal_str)){
                        int id = atoi(token_iter.GetToken(goal_str));
                        Log(info, "id: "+id);
                        if(id == val){
...
void ReceiveMessage(string msg) {
       ...
       token_iter.FindNextToken(msg))
    {
		...
		///////////////
		if(sub_msg == "enable" && token_iter.FindNextToken(msg)){
                SetEnabledCharacterAndItems(atoi(token_iter.GetToken(msg)),true);
        }else if(sub_msg == "disable" && token_iter.FindNextToken(msg)){
                SetEnabledCharacterAndItems(atoi(token_iter.GetToken(msg)),false);
        }else if(sub_msg == "play_sting" && token_iter.FindNextToken(msg)){
                Log(info, "playin' flowin' stin' "+atoi(token_iter.GetToken(msg)));
                PlayRandSting(atoi(token_iter.GetToken(msg)));
            
        }else if(sub_msg == "plysng" && token_iter.FindNextToken(msg)){
			PlaySong(token_iter.GetToken(msg));
			if(token_iter.FindNextToken(msg)){ SetMusicLayer(atoi(token_iter.GetToken(msg)));}
		}else if(sub_msg == "strttmr"){
			if(!EditorModeActive()){
				ttimermng = the_time + ttimermng;
				on__time = true;
			}
		//////////////
        }else if(sub_msg == "music_layer_override"){
            if(token_iter.FindNextToken(msg)){
   ...
   if(token == "knocked_over" ||
    ...
    if(type == kOneInt){
		...
		/////
		if (GetPlayerCharacterID() != -1 && char_a == ReadCharacter(GetPlayerCharacterID()).GetID() && on__achievment){
			omgachhivgoalsarkinnacmplctdt(token, char_a, -1);
		}
		//Log(info,"plyer id = " + ReadCharacter(GetPlayerCharacterID()).GetID());
		////
		...
		/////
		if (GetPlayerCharacterID() != -1 && char_a == GetPlayerCharacterID() && on__achievment){
			omgachhivgoalsarkinnacmplctdt(token, char_a, char_b);
		}
		////
        if(token == "knocked_over"){
...
void SetMusicLayer(int layer){
	//higestlyer = max(higestlyer,layer);
    if(kDebugText){
...
void PreDraw(float curr_game_time) {
	...
	///
	if(on__time && the_time >= ttimermng && !EditorModeActive()){
			if(params.HasParam("goal_"+progress+"_lost")){
				TriggerGoalString(params.GetString("goal_"+progress+"_lost"),_all_but_sting);
			} else {
				level.SendMessage("reset"); 
    ...
    if(params.HasParam("player_spawn")){
        ...
        while(token_iter.FindNextToken(param_str)){
            ...
            if(ObjectExists(id)){
                ...
                if(obj.GetType() == _placeholder_object){
                    PlaceholderObject@ placeholder_object = cast<PlaceholderObject@>(obj);
                    placeholder_object.SetPreview("Data/Objects/characters/rabbot.xml");
                    ++num_player_spawn;
                }
            }
        }
...
uint char_limit = 256;
void UpdateIngameText(string str, float rot) {
    TextCanvasTexture @text = level.GetTextElement(ingame_text_id);
    text.ClearTextCanvas();
    string font_str = "Data/Textures/ComicSansenviedevivre.ttf";
    ...
    text.SetPenColor(255,0,255,255);
    text.SetPenRotation(rot);
...
void Draw() {
	if(on__time && !EditorModeActive()){
		if(ingame_text_id == -1){
			TextMetrics txtmtrcs = GetTextAtlasMetrics("Data/Textures/ComicSansenviedevivre.ttf", 128, 0, int(ttimermng - the_time) + " secs");
			ingame_text_id = TextInit(txtmtrcs.bounds_x,128);
		}
		rotation = 0.0;
        UpdateIngameText(int(ttimermng - the_time) + " secs", rotation);
        HUDImage @image = hud.AddImage();
        image.SetImageFromText(level.GetTextElement(ingame_text_id));
        image.position.x = 20;//man i looooooooooooove bitcoeeeeeeweeeeeeeeec
        image.position.z = 3;
        image.color = vec4(1,1,1,1);
	}
	if(on__achievment && !EditorModeActive()){
		if(ingame_text_id == -1){
			ingame_text_id = TextInit(400,256);
		}
		rotation = 0.0;
        UpdateIngameText(nailedittimes+"/"+achievrepeats+"\n"+spaduzzledittimes+"/"+strikerepeats, rotation);
        HUDImage @image = hud.AddImage();
        image.SetImageFromText(level.GetTextElement(ingame_text_id));
        image.position.x =  GetScreenWidth()-400;//man i looooooooooooove bitcoeeeeeeweeeeeeeeec
        image.position.z = GetScreenHeight()-256;
		image.position.y = 10;
        image.color = vec4(1,1,0.45,1);
	}
}

void DrawEditor(){
    Object@ obj = ReadObjectFromID(hotspot.GetID());
    DebugDrawBillboard("Data/images/COGC2018/MNG_Dialogue/loadpreview.png",
                       obj.GetTranslation(),
                       obj.GetScale()[1]*2.0,
                       vec4(vec3(0.5), 0.8),
                       _delete_on_draw);
