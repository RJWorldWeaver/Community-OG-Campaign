...
void SetParameters() {
	...
	params.AddIntCheckbox("Only once", true);
	params.AddIntCheckbox("Ignore combat state", false);
...
void TryToPlayDialogue() {
    if(!played || (params.GetInt("Only once") == 0 && exited)){
		exited = false;
        bool player_in_valid_state = params.GetInt("Ignore combat state") == 1;
        for(int i=0, len=GetNumCharacters(); i<len && !player_in_valid_state; ++i){
        ...
        if(player_in_valid_state){
            if(!params.HasParam("Fade") || params.GetInt("Fade") == 1){
                level.SendMessage("cutcn_fade \""+params.GetString("Dialogue")+"\"");
            } else {
                level.SendMessage("cutcn \""+params.GetString("Dialogue")+"\"");                
            }
            played = true;
        }
    }
...
void OnExit(MovementObject @mo) {
 exited = mo.controlled;
}

void Draw() {
    if(params.GetInt("Visible in game") == 1 || EditorModeActive()){
        Object@ obj = ReadObjectFromID(hotspot.GetID());
        DebugDrawBillboard("Data/images/COGC2018/MNG_Dialogue/StartDialogueBillboard.png",
                           obj.GetTranslation() + obj.GetScale()[1] * vec3(0.0f,0.5f,0.0f),
                           2.0f,
                           vec4(vec3(0.75f),0.5f),
                           _delete_on_draw);
    }
