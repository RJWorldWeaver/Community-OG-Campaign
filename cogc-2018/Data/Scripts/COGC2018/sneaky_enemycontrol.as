        ...
        if(!ignore_sound) {
            ...
            if(suspicious_amount >= 1.0f) {
                ...
                if(type == _sound_type_foley || type == _sound_type_loud_foley) {
                    ...
                    //bc whatever this is sneaky characters don't do it!
                } else if(type == _sound_type_voice && ReadCharacterID(created_by_id).GetIntVar("knocked_out") == _awake && !sneaky) {
                    if(goal == _patrol) {
                        Startle();
...
void HandleAIEvent(AIEvent event) {
    switch(event) {
        ...
        case _damaged:
        //bc no idea?
            if(goal == _patrol && combat_allowed && !sneaky) {
                Log(info, "Damaged!");
...
//bc make sneaky character not hear as well
        if(sneaky) {
            max_range *= 0.25;
        }
...
//bc sneaky makes characters less aware of enemies
                        if(sneaky && goal == _patrol) {
                            amount *= -0.1;
                        }
//bc sneaky characters become aware if they're not patrolling (aka if they're attacked)
                        if(sneaky && goal != _patrol) {
                            amount = 0.5;
                        }
...
//bc sneaky character disabling provoking attack?
        if((!sneaky && chase_allowed) || (!combat_allowed && chase_allowed && !sneaky)) {
            target_goal = _provoke_attack;
        } else if(!combat_allowed || sneaky) {
            target_goal = _defend;
        }
        ...
        case _defend:
            if(CheckRangeChange(ts)) {
                if(!combat_allowed || sneaky) { //bc Not sure but if combat is not allowed you stay further from enemies? so sneaky characters too?
...
//bc make sneaky crouch while patrolling but not when trying to jump or climb

    if(sneaky && goal == _patrol && (!has_jump_target || trying_to_climb == _jump)) {
        return true;
    }
    return false;
}
...
//bc wont want to attack if sneaky
    if((ai_attacking && !startled && combat_allowed && (!sneaky || (sneaky && goal != _patrol))) || (ai_attacking && !startled && (!sneaky || (sneaky && goal != _patrol))))  {
        return true;
    } else {
        return false;
    }
...
//bc no combat roll in sneak
    if(ragdoll_time > roll_after_ragdoll_delay && combat_allowed && !startled && goal == _attack && !sneaky) {
        return true;
    } else {
        return false;
    }
...
//bc sneaky character wont yelp or listen/look trigger?
    if(suspicious_amount > 0.0f && suspicious_amount > enemy_seen && !asleep && !sneaky) {
        if(suspicious_amount > 0.5f) {
            return "Data/Animations/r_actionidle_listen_b.anm";
        } else {
            return "Data/Animations/r_actionidle.anm";
        }
    } else if(enemy_seen > 0.0f && !asleep  && !sneaky) {
...
//bc make sure they're not sneaky
vec3 GetAttackMovement() {
    if((combat_allowed && !sneaky) || (chase_allowed && !sneaky)) {
